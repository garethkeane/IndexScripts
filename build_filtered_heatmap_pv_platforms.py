...
#20221010 Trying to do some custom labeling
...
stock_ticker_list.extend(connectivity_and_compute_list)
ticker_list.extend(enhanced_production_list)
stock_ticker_list.extend(enhanced_production_list)
ticker_list.extend(automation_list)
stock_ticker_list.extend(automation_list)
...
mega_cap_tickers = ['AAPL'] # Should be a don't care as AAPL not in list anymore
...
for ticker in connectivity_and_compute_list:
	stock_sector[ticker] = 'Connectivity & Compute'
	stock_color[ticker] ='#0E4C92'
for ticker in enhanced_production_list:
	stock_sector[ticker] = 'Enhanced Production'
	stock_color[ticker] = '#95C8D8'
for ticker in automation_list:
	stock_sector[ticker] = 'Automation'
...
for (row_label, row_series) in market_cap_dataframe.iterrows():
	...
	company_name_value = row_series[1]
#	market_cap_value = market_cap_dataframe.loc[row_label, 'Market Cap']
	deeptech_market_cap[ticker_value] = market_cap_value
	node_label = company_name_value + str(market_cap_value)
	labels_for_heatmap[ticker_value] = company_name_value
...
heatmap_figure_two = px.treemap(
	...
	values = 'Market Cap',
	#labels = labels + labels_for_heatmap,
	color = 'Color',
	#color = 'Market Cap',
...
#heatmap_figure_two.data[0].hovertemplate = '%{label}<br>', labels_for_heatmap['%{label}'], 'Market Cap = %{value}'
heatmap_figure_two.data[0].hovertemplate = '%{label}<br>Market Cap = %{value:$,.0f}<br>'
#heatmap_figure_two.update_traces(labels=labels_for_heatmap)
#heatmap_figure_two.data[0].textinfo = labels_for_heatmap
...
sector_color_map = {
        'Connectivity & Compute':'#0E4C92',
        'Enhanced Production':'#95C8D8',
        'Automation':'#598BAF',
        }
...
heatmap_figure_two.update_traces(root_color = 'lightgrey')
heatmap_figure_two.write_html('testing_deeptech_heatmap_filtered_platforms_blues.html', config = graph_config)
...
heatmap_figure_no_megacaps.for_each_trace(
  lambda t: t.update(
    ...
    #labels = [ 
    #  labels_for_heatmap[id.split("/")[1]] if len(id.split("/")==2 else c
    #  for c, id in zip(t.labels, t.ids)
    #]
  )
)
...
heatmap_figure_no_megacaps.update_traces(root_color = 'lightgrey')
heatmap_figure_no_megacaps.write_html('testing_deeptech_heatmap_filtered_platforms_no_megacaps.html', config = graph_config)
